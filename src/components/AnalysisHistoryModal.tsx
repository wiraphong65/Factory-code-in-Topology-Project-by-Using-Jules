...
const AnalysisHistoryModal: React.FC<AnalysisHistoryModalProps> = ({
  ...
  const parseDeviceTypes = (deviceTypesString: string) => {
    try {
      return JSON.parse(deviceTypesString);
    } catch {
      return {};
    }
  };
                  ...
                  {analysisHistory.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                        {Object.entries(parseDeviceTypes(item.device_types)).map(([type, count]) => (
                          <div key={type} className="flex items-center gap-1 text-xs bg-gray-100 px-2 py-1 rounded-lg">
                            <div className={`w-2 h-2 ${getDeviceTypeColor(type)} rounded-full`}></div>
