...
export const EdgePropertiesPanel: React.FC<EdgePropertiesPanelProps> = ({
  ...
  const validate = (): boolean => {
    ...
    const bandwidth = tempEdgeData?.bandwidth || '1000';
    if (isNaN(Number(bandwidth)) || Number(bandwidth) < 0) {
      newErrors.bandwidth = "Bandwidth ต้องเป็นตัวเลขบวก";
    }
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2 text-sm text-gray-600">
                  ...
                  <div>จาก: {selectedEdge.source}</div>
                  <div>ไปยัง: {selectedEdge.target}</div>
                </div>
              </CardContent>
            </Card>
