...
export const PropertiesPanel = ({ selectedNode, onClose, onUpdateNode, onDeleteNode }: PropertiesPanelProps) => {
  ...
  useEffect(() => {
    if (selectedNode) {
      ...
      setFormData({
        ...
        deviceRole: selectedNode.data.deviceRole || '',
  ...
  }, [selectedNode]); // Update whenever selectedNode changes
  ...
  const validate = (): boolean => {
    ...
    // Validate throughput (ไม่บังคับสำหรับ PC, ใช้ค่าเริ่มต้น 1000 Mbps ถ้าไม่กรอก)
    if (selectedNode && selectedNode.type.toLowerCase() !== 'pc') {
      if (formData.maxThroughput !== '' && (isNaN(Number(formData.maxThroughput)) || Number(formData.maxThroughput) < 0)) {
